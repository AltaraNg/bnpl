  ...
  <main class="px-5 w-full bg-white">
    <div class="space-y-4 relative h-fit">
      ...
      >
        <input
          type="text"
          class="w-full lg:w-1/3 rounded-full h-[44px] border"
        />
        <search class="" />
      </div>
      <div
      ...
      >
        <div v-for="order in orders" :key="order.id" class="lg:w-0.3">
          <div
            class="bg-primary w-full rounded-md shadow-xs text-white p-3 flex items-center justify-between lg:mb-4"
          >
            <div class="space-y-5 lg:space-y-0">
              <div>
                <p class="text-xs font-light">{{ order.id }}</p>
                <p>{{ order.product_name }}</p>
              </div>
              <p class="font-semibold">{{ order.customer_name }}</p>
            </div>
            <div>
              <p class="text-lg font-bold">{{ order.amount }}</p>
              <p class="text-xs font-light">{{ order.date }}</p>
            </div>
          </div>
        </div>
      </div>
